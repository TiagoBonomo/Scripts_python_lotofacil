...
lista_lfacil = open(r"C:\testearquivos\dezenasprovaveis01testetodas.txt")
...
def comparar(p_combinacoes):
    ...
    for i in range(len(combinacoes)):
        ...
        if((somaTotal >= 191) & (somaTotal <= 202)):
            if(somaDezena <= 15):
                if(somaUnidade <= 67):
                    if(somaTodos >= 70):

                        with open(r"C:\testearquivos\somaCombinacao.txt", "a") as val:
                            print(cont03)
